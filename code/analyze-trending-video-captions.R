...
vids %>%
    ...
    count(videoId, channelTitle, videoTitle, captionType) %>%
    pivot_wider(id_cols = c(videoId, channelTitle, videoTitle),
                names_from = captionType, values_from = n) %>%
    filter(asr >= 1, standard >= 1) %>%
